...
export class GoalAggregate {
  ...
  public sortGoalsByPriority = (): void => {
    this.goals = this.goals.sort((a: Goal, b: Goal) => {
      return a.Priority ? a.Priority - b.Priority : -1
    })
  }
  ...
  public GetPriorityLightWeightPercentage(goal: Goal): number {
    if (goal.Percentage) {
  ...
  public GetPriorityHeavyWeightPercentage(goal: Goal): number {
    if (goal.Percentage) {
      return goal.Percentage
    }
  ...
  public GetWeightPercentage(goal: Goal): number {
    if (goal.Percentage) {
      return goal.Percentage
    }
  ...
  private allGoalWeightNoPrioritySum = (): number => {
    const nonPercentageWeights = this.goals
      .filter(x => !x.Percentage)
      .map(x => x.GetWeight())
      .reduce((sum, element) => sum + element, 0)
    const percentageWeights = this.goals
      .filter(x => x.Percentage)
      .map(x => x.GetWeight())
      .reduce((sum, element) => sum + element, 0)
  ...
  private allGoalWeightLightPrioritySum = (): number => {
    ...
    const nonPercentageWeights = this.goals
      .filter(x => !x.Percentage)
      .map(x => x.GetWeight() * ((maxPriority - x.Priority + 1) / maxPriority))
      .reduce((sum, element) => sum + element, 0)
    const percentageWeights = this.goals
      .filter(x => x.Percentage)
      .map(x => x.GetWeight())
